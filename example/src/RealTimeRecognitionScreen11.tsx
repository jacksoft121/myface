...
import { type FaceBoxBuf, type FaceBoxUI } from './dto/DlxTypes';
import { getAllUsers, queryUserByFaceId } from './comm/FaceDB'; // 导入 FaceDB 的函数
  ...
  useEffect(() => {
    ...
    const loadRegisteredFaces = async () => {
      try {
        ...
        log(`Reloaded faces from DB: ${cnt}, hubCount: ${hubCount}`);
      } catch (e) {
        log('Reload faces from DB error:', e);
      }
    };
  ...
  const reportFacesToJS = useMemo(() => {
    ...
    return Worklets.createRunOnJS(
      async (payload: {
        ...
        const facesWithDbNames = await Promise.all(
          payload.faces.map(async (b) => {
            let name = '未注册';
            if (b.isMatched && b.faceId) {
              const user = await queryUserByFaceId(b.faceId);
              name = user?.dlx_user_name || user?.name || '未知用户';
            }
            return { ...b, name };
          })
        );

        const next: FaceBoxUI[] = facesWithDbNames.map((b) => {
